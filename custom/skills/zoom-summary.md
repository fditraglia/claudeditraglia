...
```bash
ffmpeg -i "<path-to-m4a>" -ar 16000 -ac 1 -c:a pcm_s16le "/tmp/zoom-audio.wav" -y
```
...
Run whisper-cli with the large-v3 model:
...
3. **Action items / next steps** — who needs to do what, with names if identifiable
...
- **whisper.cpp** — `~/whisper.cpp/build/bin/whisper-cli` with `ggml-large-v3.bin` model
